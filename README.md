...
```
testcases.jsonl ─┐
                 ├─▶ Stage 1: Inference ──▶ inference.jsonl
run-config.yaml ─┘         │
                           ├─▶ Stage 2: Autocheck ──▶ autocheck.jsonl
                           │
                           ├─▶ Stage 3: Judge ──▶ judgements.jsonl
                           │
                           └─▶ Stage 4: Compare ──▶ comparison-report.json / .md
```
...
| `data/judgements-{run_id}.jsonl` | JSONL | Judge の採点結果 |
